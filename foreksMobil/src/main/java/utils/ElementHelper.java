...
public boolean CompareDescList(By locator){
        List<WebElement>eList=findElements(locator);
    for (int i = 0; i <eList.size()-1 ; i++) {
        double current = Double.parseDouble(eList.get(i).getText().replace("%", "").replace(",", "."));
        double next = Double.parseDouble(eList.get(i+1).getText().replace("%", "").replace(",", "."));
    ...
    public boolean CompareAscList(By locator){
        List<WebElement>eList=findElements(locator);
        for (int i = 0; i <eList.size()-1 ; i++) {
            double current = Double.parseDouble(eList.get(i).getText().replace("%", "").replace(",", "."));
            double next = Double.parseDouble(eList.get(i+1).getText().replace("%", "").replace(",", "."));
